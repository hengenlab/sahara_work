...
for i, crit in enumerate(caf22_objs):
    ...
    for ip, p in enumerate(all_perc):
        print(f"\n\n--SETTING PERC -- {p}\n\n")
        crit.perc = float(p/100)
        for ib, b in enumerate(all_bins):
            print(f"\n\n--SETTING BINSIZE -- {b}\n\n")
            crit.ava_binsize = float(b/1000)
            ...
            try:
                crit = s.run_crit_from_start(crit,save=False)
            except Exception:
                    print('TIMEOUT or ERROR', flush = True)
                    errors.append(f'{crit.animal} -- {crit.probe} -- {crit.date} -- {crit.time_frame} -- {i} --- ERRORED')
                    noerr=False
            signal.alarm(0)
            if noerr:
