...
class Neuron:
    ...
    def checkqual(self, binsz=3600, start=False, end=False):
        ...
        with plt.style.context('seaborn-dark-palette'):
            fig = plt.figure(constrained_layout=True, figsize=(12, 8)) # unsure what the constrained layout thing does
            gs = fig.add_gridspec(3,4)

            waveform_ax = fig.add_subplot(gs[0:-1, 0])
            waveform_ax.set_title("Waveform")

            isi_ax = fig.add_subplot(gs[2,0])
            isi_ax.set_title("ISI Hist")

            amp_ax = fig.add_subplot(gs[0,1:-1])
            amp_ax.set_title("Amplitudes")

            fr_ax = fig.add_subplot(gs[1, 1:-1])
            fr_ax.set_title("Firing Rate")

            isi_time_ax = fig.add_subplot(gs[-1, 1:-1])
            isi_time_ax.set_title("Isi over time")

            qual_ax = fig.add_subplot(gs[0,-1])
