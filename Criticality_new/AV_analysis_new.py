...
def scaling_plots(burst, burstMin, burstMax, alpha, T, tMin, beta, TT, Sm, sigma, fit_sigma, plt_name, saveloc):
    # burst PDF
...
def AV_analysis_new(burst, T, bm, tm, pltname, saveloc, flag = 1, burst_shuffled=None, T_shuffled=None, plot_shuffled=False, plot=True):
    ...
    burst_idx = np.where(np.logical_and(burst<=burstMax, burst>=burstMin))[0]
